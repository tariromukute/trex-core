...
CSctpStats::CSctpStats(uint16_t num_of_tg_ids) {
    Init(num_of_tg_ids);
}

CSctpStats::~CSctpStats() {
    delete[] m_sts_tg_id;
}

void CSctpStats::Init(uint16_t num_of_tg_ids){
    m_sts_tg_id = new udp_stat_int_t[num_of_tg_ids];
    assert(m_sts_tg_id && "Operator new failed in udp.cpp/CSctpStats::Init");
    m_num_of_tg_ids = num_of_tg_ids;
    Clear();
}

void CSctpStats::Clear(){
...
void CSctpStats::ClearPerTGID(uint16_t tg_id) {
    memset(m_sts_tg_id+tg_id,0,sizeof(udp_stat_int_t));
}

void CSctpStats::Dump(FILE *fd){
}

void CSctpStats::Resize(uint16_t new_num_of_tg_ids){
    delete[] m_sts_tg_id;
    Init(new_num_of_tg_ids);
}

void CSctpFlow::Create(CPerProfileCtx *pctx,
...
HOT_FUNC void CSctpFlow::update_checksum_and_lenght(CFlowTemplate *ftp,
                                          rte_mbuf_t * m,
...
HOT_FUNC rte_mbuf_t   * CSctpFlow::alloc_and_build(CMbufBuffer *      buf){
...
void CSctpFlow::send_pkt(CMbufBuffer *      buf){
...
void CSctpFlow::keepalive_timer_start(bool init){
    if (init) {
        m_keepalive=1;
...
void CSctpFlow::set_keepalive(uint64_t msec, bool rx_mode){
...
void CSctpFlow::on_rx_packet(struct rte_mbuf *m,
                            UDPHeader *udp,
                            int offset_l7,
                            int total_l7_len){
